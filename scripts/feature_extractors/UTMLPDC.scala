...
//######################################################################################
//######################################################################################
def extract_CDF_UTMLPDC(in: String, res: String, minDeg: Int = 0, maxDeg: Int = Int.MaxValue, minMsgCnt: Int = 0, maxMsgCnt: Int = Int.MaxValue, minPar: Int, pid: Int, minParSize: Int) = {
...
if(pid != -1 )
 ML_filt_deg_cnt = UTMLPDC.filter({case(u, (t, ml, p, deg, cnt)) => deg > minDeg && deg < maxDeg && cnt > minMsgCnt && cnt < maxMsgCnt && p.toInt == pid})

val split = ML_filt_deg_cnt.map({case(u,(top, ml, p, deg, cnt)) => (p,u)}).groupByKey().map(x => (x._1, x._2.toList.distinct)).filter(_._2.size > minParSize).map({case(p,u) => (p, u.splitAt((u.size * 0.7).toInt))})
...
val Recall = (U_PE.count * 1.0/ML_filt_deg_cnt.join(test).count).toDouble
