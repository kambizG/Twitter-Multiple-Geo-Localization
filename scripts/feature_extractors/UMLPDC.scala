...
val stats = sc.textFile(in).map(_.split(",")).map(x => (x(0), ((x(1).toDouble, x(2).toDouble), x(3), x(4).toInt, x(5).toInt)))
...
val Recall = (train.count + test.count) * 1.0/ stats.groupByKey().count
