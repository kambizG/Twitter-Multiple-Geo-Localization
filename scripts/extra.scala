...
//######################################################################################
def extract_valid_partitions(partitions: String, stats: String, min_avg_msg_cnt: Int, max_avg_msg_cnt: Int, day: Int, time: Int, min_par_cnt: Int, min_par_msg_cnt: Int, max_par_msg_cnt: Int): org.apache.spark.rdd.RDD[(String, Double)]= {
...
val par_msg_ratio = par_msg_cnt.join(part_count).map(x => (x._1, x._2._1 * 1.0 / x._2._2)).filter(x => x._2 > min_par_msg_cnt && x._2 < max_par_msg_cnt)
